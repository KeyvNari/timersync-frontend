...
export function WebSocketProvider({
  ...
  const currentConnectionRef = useRef<ConnectionInfo | null>(null);
  ...
  const setupEventHandlers = (wsService: SimpleWebSocketService) => {
    ...
    wsService.on('identify_request', (message: any) => {
      // Show the overlay with the current connection name
      const connectionName = currentConnectionRef.current?.connection_name || 'Your Connection';

      setIdentifyNotification({ connectionName });
